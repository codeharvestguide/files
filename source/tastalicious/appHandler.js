...
function navScrollEffect(navElement) {
    if (window.scrollY > 0) {
        navElement.style.boxShadow = '0 0 25px rgba(177, 177, 177, 0.1)';
        navElement.style.paddingTop = '20px';

    }
    else {
        navElement.style.boxShadow = 'none';
        navElement.style.paddingTop = '25px'
    }
}
...
function addToCart(userCart, bttn) {
    ...
    let addItemImage = addItem.querySelector('.image-container img').src;
            ...
            <div class="cart-item">
                <div class="item-1">
                    <div class="cart-item-image">
                        <img src="${addItemImage}" alt="${addItemName}">
...
function updateTotalPrice(itemPrice, operation) {
    ...
    totalPrice = (operation == "addItem") ? (totalPrice + itemPrice) :
                (operation == "removeItem") ? (totalPrice - itemPrice) : 0
...
function newCartItemEffect() {
    ...
    let cartButton = document.querySelector('#page-heading .cart-button');
